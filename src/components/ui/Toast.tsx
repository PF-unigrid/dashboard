// src/components/ui/Toast.tsx

import React, { useState, useEffect } from 'react';
import { create } from 'zustand';
import type { NotificationConfig, NotificationAction } from '../../types/domain/common.types';

/**
 * Estado del sistema de notificaciones
 */
interface ToastState {
  toasts: Toast[];
  addToast: (config: NotificationConfig) => string;
  removeToast: (id: string) => void;
  clearAllToasts: () => void;
}

interface Toast extends NotificationConfig {
  id: string;
  createdAt: Date;
}

/**
 * Store para el sistema de notificaciones
 */
export const useToastStore = create<ToastState>((set) => ({
  toasts: [],

  addToast: (config) => {
    const id = `toast_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const toast: Toast = {
      ...config,
      id,
      createdAt: new Date(),
      duration: config.duration ?? 5000, // 5 segundos por defecto
    };

    set((state) => ({
      toasts: [...state.toasts, toast]
    }));

    // Auto-remove si tiene duración
    if (toast.duration && toast.duration > 0) {
      setTimeout(() => {
        set((state) => ({
          toasts: state.toasts.filter(t => t.id !== id)
        }));
      }, toast.duration);
    }

    return id;
  },

  removeToast: (id) => {
    set((state) => ({
      toasts: state.toasts.filter(t => t.id !== id)
    }));
  },

  clearAllToasts: () => {
    set({ toasts: [] });
  }
}));

/**
 * Hook para usar notificaciones de manera sencilla
 */
export const useToast = () => {
  const { addToast, removeToast } = useToastStore();

  const toast = {
    success: (title: string, message?: string, actions?: NotificationAction[]) =>
      addToast({ type: 'success', title, message: message || '', actions }),

    error: (title: string, message?: string, actions?: NotificationAction[]) =>
      addToast({ type: 'error', title, message: message || '', actions }),

    warning: (title: string, message?: string, actions?: NotificationAction[]) =>
      addToast({ type: 'warning', title, message: message || '', actions }),

    info: (title: string, message?: string, actions?: NotificationAction[]) =>
      addToast({ type: 'info', title, message: message || '', actions }),

    custom: (config: NotificationConfig) => addToast(config),

    dismiss: (id: string) => removeToast(id)
  };

  return toast;
};

/**
 * Componente Toast individual
 */
const ToastItem: React.FC<{ toast: Toast }> = ({ toast }) => {
  const [isVisible, setIsVisible] = useState(false);
  const [isLeaving, setIsLeaving] = useState(false);
  const removeToast = useToastStore(state => state.removeToast);

  useEffect(() => {
    // Animación de entrada
    setTimeout(() => setIsVisible(true), 10);
  }, []);

  const handleClose = () => {
    setIsLeaving(true);
    setTimeout(() => removeToast(toast.id), 150);
  };

  const getToastStyles = () => {
    const baseStyles = 'transition-all duration-150 ease-out';
    const visibilityStyles = isVisible && !isLeaving
      ? 'transform translate-x-0 opacity-100'
      : 'transform translate-x-full opacity-0';

    const typeStyles = {
      success: 'bg-green-50 border-green-200 text-green-800',
      error: 'bg-red-50 border-red-200 text-red-800',
      warning: 'bg-yellow-50 border-yellow-200 text-yellow-800',
      info: 'bg-blue-50 border-blue-200 text-blue-800'
    };

    return `${baseStyles} ${visibilityStyles} ${typeStyles[toast.type]}`;
  };

  const getIconForType = () => {
    switch (toast.type) {
      case 'success':
        return (
          <svg className="w-5 h-5 text-green-400" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
          </svg>
        );
      case 'error':
        return (
          <svg className="w-5 h-5 text-red-400" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
          </svg>
        );
      case 'warning':
        return (
          <svg className="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
          </svg>
        );
      case 'info':
        return (
          <svg className="w-5 h-5 text-blue-400" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
          </svg>
        );
    }
  };

  return (
    <div className={`max-w-sm w-full border rounded-lg shadow-lg p-4 ${getToastStyles()}`}>
      <div className="flex items-start">
        <div className="flex-shrink-0">
          {getIconForType()}
        </div>
        
        <div className="ml-3 w-0 flex-1 pt-0.5">
          <p className="text-sm font-medium">
            {toast.title}
          </p>
          {toast.message && (
            <p className="mt-1 text-sm opacity-90">
              {toast.message}
            </p>
          )}
          
          {toast.actions && toast.actions.length > 0 && (
            <div className="mt-3 flex space-x-2">
              {toast.actions.map((action, index) => (
                <button
                  key={index}
                  onClick={() => {
                    action.handler();
                    handleClose();
                  }}
                  className={`text-sm font-medium px-2 py-1 rounded ${
                    action.variant === 'primary'
                      ? 'bg-white bg-opacity-20 hover:bg-opacity-30'
                      : 'hover:bg-white hover:bg-opacity-10'
                  }`}
                >
                  {action.label}
                </button>
              ))}
            </div>
          )}
        </div>
        
        <div className="ml-4 flex-shrink-0 flex">
          <button
            onClick={handleClose}
            className="inline-flex text-current hover:opacity-75 focus:outline-none focus:opacity-75"
          >
            <span className="sr-only">Close</span>
            <svg className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
            </svg>
          </button>
        </div>
      </div>
    </div>
  );
};

/**
 * Contenedor de todas las notificaciones
 */
export const ToastContainer: React.FC = () => {
  const toasts = useToastStore(state => state.toasts);

  if (toasts.length === 0) {
    return null;
  }

  return (
    <div 
      className="fixed top-0 right-0 z-50 p-4 space-y-4 max-h-screen overflow-hidden"
      aria-live="assertive"
    >
      {toasts.map((toast) => (
        <ToastItem key={toast.id} toast={toast} />
      ))}
    </div>
  );
};